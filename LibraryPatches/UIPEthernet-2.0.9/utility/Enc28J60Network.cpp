...
#define USE_STM32F1_DMAC 1 //on STM32
  ...
  if (timeout == 0)
  {
    LogObject.uart_send_strln(F("ENC28J60::init ERROR:TIMEOUT !!"));
  }
  ...
  LogObject.uart_send_strln(
    F("ENC28J60::init DEBUG_V3:After writeReg(ERXFCON, ERXFCON_UCEN|ERXFCON_CRCEN|ERXFCON_PMEN|ERXFCON_BCEN)"));
#endif
#if defined(ESP8266)
  wdt_reset();
#endif
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("ENC28J60::init DEBUG_V3:After writeOp(ENC28J60_BIT_FIELD_SET, MACON3, MACON3_PADCFG0|MACON3_TXCRCEN|MACON3_FRMLNEN)"));
#endif
...
Enc28J60Network::receivePacket(void)
{
  ...
  {
    LogObject.uart_send_strln(
      F("Enc28J60Network::receivePacket(void) ERROR:ENC28j50 Device not found !! Bypass receivePacket function !!"));
  }
  ...
  {
    uint16_t readPtr = nextPacketPtr + 6 > RXSTOP_INIT ? nextPacketPtr + 6 - ((RXSTOP_INIT + 1) - RXSTART_INIT) :
                       nextPacketPtr + 6;
...
#endif
  return ((handle == NOBLOCK)
          || (erevid == 0)) ? 0 : handle == UIP_RECEIVEBUFFERHANDLE ? receivePkt.size : blocks[handle].size;
}
...
Enc28J60Network::sendPacket(memhandle handle)
{
...
#if ACTLOGLEVEL>=LOG_ERR
    LogObject.uart_send_strln(
      F("Enc28J60Network::sendPacket(memhandle handle) ERROR:ENC28j50 Device not found !! Bypass sendPacket function !!"));
#endif
    return false;
  }
  ...
  uint16_t end = start + packet->size - 1 -
                 UIP_SENDBUFFER_PADDING; // end = start + size - 1 and padding for TSV is no included
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("Enc28J60Network::setReadPtr(memhandle handle, memaddress position, uint16_t len) DEBUG_V3:Function started"));
#endif
  memblock *packet = handle == UIP_RECEIVEBUFFERHANDLE ? &receivePkt : &blocks[handle];
  memaddress start = handle == UIP_RECEIVEBUFFERHANDLE
                     && packet->begin + position > RXSTOP_INIT ? packet->begin + position - ((RXSTOP_INIT + 1) - RXSTART_INIT) :
...
Enc28J60Network::readPacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len)
{
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("Enc28J60Network::readPacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) DEBUG_V3:Function started"));
#endif
#if defined(ESP8266)
  wdt_reset();
#endif
  ...
  LogObject.uart_send_str(
    F("Enc28J60Network::readPacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) DEBUG_V2: Read bytes:"));
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_str(
    F("Enc28J60Network::writePacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) DEBUG_V3:Function started with len:"));
  LogObject.uart_send_decln(len);
#endif
#if defined(ESP8266)
  wdt_reset();
...
#if ACTLOGLEVEL>=LOG_DEBUG_V2
  LogObject.uart_send_str(
    F("Enc28J60Network::writePacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) DEBUG_V2: Write bytes:"));
  LogObject.uart_send_dec(len);
  LogObject.uart_send_str(F(" save to block("));
  LogObject.uart_send_dec(handle);
  LogObject.uart_send_str(F(") ["));
...
void
Enc28J60Network::copyPacket(memhandle dest_pkt, memaddress dest_pos, memhandle src_pkt, memaddress src_pos,
                            uint16_t len)
{
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("Enc28J60Network::copyPacket(memhandle dest_pkt, memaddress dest_pos, memhandle src_pkt, memaddress src_pos, uint16_t len) DEBUG_V3:Function started"));
#endif
  ...
  memaddress start = src_pkt == UIP_RECEIVEBUFFERHANDLE
                     && src->begin + src_pos > RXSTOP_INIT ? src->begin + src_pos - ((RXSTOP_INIT + 1) - RXSTART_INIT) : src->begin +
                     src_pos;
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("enc28J60_mempool_block_move_callback(memaddress dest, memaddress src, memaddress len) DEBUG_V3:Function started"));
#endif
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("Enc28J60Network::writeOp(uint8_t op, uint8_t address, uint8_t data) DEBUG_V3:Function started"));
#endif
  CSACTIVE;
  // issue write command
#if ENC28J60_USE_SPILIB
...
#if ACTLOGLEVEL>=LOG_DEBUG_V3
  LogObject.uart_send_strln(
    F("Enc28J60Network::chksum(uint16_t sum, memhandle handle, memaddress pos, uint16_t len) DEBUG_V3:Function started"));
#endif
  uint16_t t;
